...
def main():
    ...
    search_space = {
        ...
        # MLP parameters
        "input_neurons": hp.choice("input_neurons", [8, 16, 32, 64, 128]),
        "dropout": hp.choice("dropout", [0.00, 0.10, 0.20, 0.30, 0.40, 0.50]),
        "single_layer": hp.choice("single_layer", [True, False])
    }
    ...
    def cross_val(params):
        ...
        def create_model(input_size, output_size, input_neurons, dropout, single_layer=True):
            model = Sequential()
            model.add(Dense(input_neurons, input_dim=input_size, activation="relu"))
            if dropout > 0:
                model.add(Dropout(dropout))
            current_neurons = input_neurons / 2

            if not single_layer:
                while current_neurons > 4:
        ...
        alpha_betas = [(0.9, 0), (1.1, 0), (1.0, -50),
                       (1.0, 50)]  # Change the intensity and contrast by each pair (alpha, beta): img*alpha + beta
        sol_thresholds = [
            255]  # [130, 140, 150, 160]  # Invert the intensity of the pixels above each of these thresholds
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            # Create the model
            clf = create_model(input_size=n_feat, output_size=n_class, input_neurons=params["input_neurons"], dropout=params["dropout"], single_layer=params["single_layer"])
            ...
            clf.fit(np.concatenate((X_train[:, best_feats][train_indices.ravel()], X_train_da[:, best_feats])),
                    np.concatenate((y_train[train_indices.ravel(), :], y_train_da)),
                    validation_data=(X_train[:, best_feats][validation_indices.ravel()],
            ...
            score = test_and_evaluate_model(clf, enc, X_test[:, best_feats], y_test, names_test,
