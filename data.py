...
def get_dataset_augmented(img_names, img_labels, img_weights, orientations=[0], zooms=[1.0], alpha_betas=[(1.0, 0)], sol_thresholds=[255], **kwargs):
    photo_dir = kwargs["photo_dir"]

    # Data augmentation parameters dictionary
    da_dic = {
        "orientations": orientations,
        "zooms": zooms,
        "alpha_betas": alpha_betas,
        "sol_thresholds": sol_thresholds
    }
    ...
    names_da = []
    weights_da = []
    for idx, name in enumerate(img_names.ravel()):
        or_img = cv2.imread(os.path.join(photo_dir, name), cv2.IMREAD_GRAYSCALE)
        for transformation in da_dic.keys():
            values = da_dic[transformation]
            for value in values:

                if transformation == "orientations":
                    if value == 0:
                        continue
                    img = rotate(delete_black_regions(or_img), value, resize=False, mode="symmetric")
                elif transformation == "zooms":
                    if value == 1.0:
                        continue
                    img = rescale(or_img, value)
                elif transformation == "alpha_betas":
                    if list(value) == [1.0, 0]:
                        continue
                    img = (value[0]*or_img + value[1]).astype(np.uint8)
                    img = np.maximum(0, np.minimum(255, img))
                elif transformation == "sol_thresholds":
                    if value == 255:
                        continue
                    sol_indices = np.where(or_img > value)
                    img = np.copy(or_img)
                    img[sol_indices] = 255 - img[sol_indices]
                ...
                X_da.append(np.concatenate((glcm_features[0, :], lbps_features[0, :])))
                y_da.append(img_labels[idx])
                weights_da.append(img_weights[idx])
                names_da.append([name])

    if len(X_da) > 0:
        return np.array(X_da), np.array(y_da), np.array(weights_da), np.array(names_da)
...
def test_and_evaluate_model(model, class_encoder, x, y, names, photo_dir, sample_weights=None, results_path="results", show_matrix=True):
