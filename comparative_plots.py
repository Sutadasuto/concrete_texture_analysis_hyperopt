...
from config import distances, angles, standardize_glcm_image, glcm_levels, props
from data import delete_black_regions  # GLCM parameters (number_of_features = len(distances) * len(angles) * len(props))
...
for ax_idx, img_path in enumerate(imgs):
    x_gray = delete_black_regions(cv2.imread(img_path, cv2.IMREAD_GRAYSCALE))
